...
import type { Trigger } from "../services/hercule-server/hercule-api.types";
import type {
	Message,
	MessageResponse,
...
import type { ConnectMessage } from "../types/messages.type";
import type { ListTriggersMessageResponse } from "../types/messages.type";
...
const registerEvents = async () => {
	...
	for (const service of triggerEventServices) {
		service.register();
	}
};

const onTriggerEventMessage = async (
	message: TriggerEventMessage,
): Promise<TriggerEventMessageResponse> => {
	...
	if (service) {
		...
		if (shouldTrigger) {
			const response = await service.triggerEvent(
				message.payload.event.context,
			);
			return {
...
const onListTriggersMessage =
	async (): Promise<ListTriggersMessageResponse> => {
